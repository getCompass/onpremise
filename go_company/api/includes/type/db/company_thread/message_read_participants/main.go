...
func InsertArray(ctx context.Context, tableShard int, transactionItem *sql.Tx, messageReadParticipantList []Row) error {
	...
	for _, messageReadParticipant := range messageReadParticipantList {
		...
		insertList = append(insertList, []interface{}{
			messageReadParticipant.ThreadMap,
			messageReadParticipant.ThreadMessageIndex,
	...
	query, values := mysql.InsertArray(getTableName(tableShard), []string{"thread_map", "thread_message_index", "user_id", "read_at", "message_created_at", "created_at", "updated_at", "message_map"}, insertList)
