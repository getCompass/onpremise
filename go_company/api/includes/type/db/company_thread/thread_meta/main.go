...
const tableKey = "thread_meta"

// GetByConversationForUpdate получить несколько записей из чата на обновление
func GetAllUsers(ctx context.Context, companyId int64, dbConn *company_thread.DbConn, threadMapIdList map[int][]int64) (map[string]UsersRow, error) {

	if len(threadMapIdList) == 0 {
	...
	metaIdCount := 0
	for year, metaIdList := range threadMapIdList {
	...
	threadMetaUsersMap := make(map[string]UsersRow, metaIdCount)
	...
	for results.Next() {
		...
		if err != nil {
			...
			log.Errorf("Ошибка парсинга пользователей из меты треда с id %d, компания %d", row.MetaId, companyId)
			continue
		}

		threadMetaUsersMap[GetRowKey(row.MetaId, row.Year)] = row
	}

	return threadMetaUsersMap, nil
}
