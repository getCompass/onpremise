...
func bindPort(routineChan chan *routine.Status, portValue int32, companyId int64, nonExistingDataDirPolicy, duplicateDataDirPolicy int32) {
	...
	isAlive := waitPortAlive(port, needInit, 30)

	if !isAlive {

		routineChan <- routine.MakeRoutineStatus(routine.StatusError, fmt.Sprintf("Не смогли поднять порт %s за 30 секунд", portValue))
		return
	}
