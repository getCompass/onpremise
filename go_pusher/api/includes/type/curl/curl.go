...
func SimplePost(requestUrl string, requestMap map[string]string) ([]byte, bool) {
	...
	requestItem, err := _getPostRequestItem(requestUrl, requestMap)
	if err != nil {
	...
	clientItem, err := _getPostClientItem()
	if err != nil {
	...
	return _prepareResponse(responseItem)
}

// получаем post-объект запроса
func _getPostRequestItem(requestUrl string, requestMap map[string]string) (*http.Request, error) {
...
func _getPostClientItem() (http.Client, error) {
	...
	if config.Tls {
		...
		err := _addCertificateToPostClient(&clientItem)
		if err != nil {
...
func _addCertificateToPostClient(clientItem *http.Client) error {
...
func _prepareResponse(responseItem *http.Response) ([]byte, bool) {
