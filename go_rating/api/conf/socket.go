...
func getSocketConfigFromFile(path string) (SocketConfigStruct, error) {
	...
	if err != nil {
		return SocketConfigStruct{}, fmt.Errorf("unable read file socket.json, error: %v", err)
	}
	...
	if err != nil {
		return SocketConfigStruct{}, fmt.Errorf("unable decode file socket.json, error: %v", err)
	}
