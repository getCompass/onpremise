...
type StatusInfoStruct struct {
	...
	OnlineUsers int    `json:"online_user_count"`
}

// собираем информацию о состоянии системы
func Status(companyEnvList *Isolation.CompanyEnvList) (StatusInfoStruct, error) {

	// читаем память
	memStats := runtime.MemStats{}
	runtime.ReadMemStats(&memStats)

	// считаем, сколько активных человек в каждой компании - вот и общее коилчество соединений
	onlineUsers := 0
	for _, companyIsolation := range companyEnvList.EnvList {
		onlineUsers += companyIsolation.UserConnectionStore.GetOnlineUserCount()
	}
