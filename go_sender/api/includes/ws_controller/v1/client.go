...
type clientTypingRequest struct {
	WSData struct {
		...
		TalkingHash     string  `json:"talking_hash"`
		Type            int     `json:"type"`
...
func (client) Typing(data *dataStruct) {
	...
	// проверяем hash сущности, в которую необходимо отправить
	typingHash := getTypingHash(isolation.GetGlobalIsolation().GetConfig().CustomSalt, requestData.WSData.UserList)
	if typingHash != requestData.WSData.TalkingHash {

		// помечаем ошибку для соединения
		ws.AppendError(data.connection, fmt.Errorf("not equal conversation hash"))
		Error(data.connection, 100, "not equal conversation hash")
		return
	}
