...
var allowHandlers = handlerMap{
	"userbot": userbotMethods,
}

// вспомогательные типы даннных
type handlerMap map[string]methodMap
type methodMap map[string]func([]byte) ResponseStruct
...
func work(method string, request []byte) ResponseStruct {
	...
	for method, item := range allowHandlers[controllerName] {
