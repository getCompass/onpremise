...
interface IProps extends AbstractProps {
    ...
    /**
     * The name of the participant currently on stage (if any).
     */
...
class Conference extends AbstractConference<IProps, State> {
    ...
    _renderContent() {
        ...
        // Если локальная миниатюра должна отображаться отдельно или скрыли себя
        if (shouldDisplayLocalThumbnailSeparately() || _disableSelfView) {
            // +1 потому что мы не в массиве участников
            participantsCount = (_participants as ArrayLike<String>).length + 1;
            // Если портретный режим
        } else if (isNarrowAspectRatio) {
            let participants = [ ..._participants, _localParticipantId ];
        ...
        let participantWidth = SMALL_THUMBNAIL_SIZE;
        // Ширина имени
        // Размер шрифта * соотношение сторон шрифта * длина текста
        let nameWidth = 14 * 0.74 * _largeVideoParticipantName.length;
        // Ширина бейджа с именем
        let participantNameWidth = nameWidth + 12;
        // Отступ между миниатюрами
        let participantOffset = 4;
        // Сумма отступов между миниатюрами
        let allParticipantOffset = (participantsCount - 1) * participantOffset;
        // Сумма ширины всех элементов
        let allWidths = 8 + participantNameWidth + 8;
            + (participantsCount * participantWidth) + allParticipantOffset + 8;
        // Нужен ли кастомный стиль для отображения бейджа имени
        // Если сумма всех широт меньше либо равна ширины экрана и сейчас портретный режим
        let isNeedCustomStyle = (allWidths <= screenWidth) && isNarrowAspectRatio;
