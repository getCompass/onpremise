...
import AvatarMobile from "../../../base/avatar/components/AvatarMobile";
import ThumbnailBottomIndicatorsMobile from "./ThumbnailBottomIndicatorsMobile";
import ThumbnailTopIndicatorsMobile from "./ThumbnailTopIndicatorsMobile";

/**
 * The type of the React {@code Component} state of {@link ThumbnailMobile}.
 */
...
/**
 * The type of the React {@code Component} props of {@link ThumbnailMobile}.
 */
export interface IProps extends WithTranslation {
    ...
    /**
     * The height of the ThumbnailMobile.
     */
    _height: number;
    ...
    _isDominantSpeakerAudioMuted: boolean;
}
...
const defaultStyles = (theme: Theme) => {
    return {
        ...
        indicatorsBottomContainerCenter: {
            justifyContent: 'center',
            padding: '0px 8px 8px 8px',
        },

        indicatorsTopContainerBetween: {
        ...
        indicatorsBackgroundBottomFlexShrink0: {
            backgroundColor: 'rgba(33, 33, 33, 0.9)',
            borderRadius: '6px',
            display: 'flex',
            flexShrink: 0,
            ...
            '&:not(:empty)': {
                padding: '4.5px 8px',
                maxWidth: 'calc(100% - 16px)', // 100% - padding_left - padding_right
            },
        ...
        indicatorsBackgroundTopFlexShrink0: {
            backgroundColor: 'rgba(33, 33, 33, 0.9)',
            borderRadius: '6px',
            display: 'flex',
            flexShrink: 0,
        ...
        activeSpeaker: {
            '& .active-speaker-indicator': {
                boxShadow: `inset 0px 0px 0px 2px #006be0!important`
            }
        },

        raisedHand: {
            '& .raised-hand-border': {
                boxShadow: `inset 0px 0px 0px 2px rgba(255, 214, 56, 1) !important`
            }
        },
 ...
 * @augments Component
 */
class ThumbnailMobile extends Component<IProps, IState> {
    /**
    ...
    /**
     * Initializes a new ThumbnailMobile instance.
     *
    ...
    componentDidUpdate(prevProps: IProps, prevState: IState) {
        ...
        // TODO: after converting this component to a react function component,
        // use a custom hook to update local track streaming status.
        const { _videoTrack, dispatch, _participant } = this.props;
     ...
     */
    _getAvatarSize() {
        const { _height, _width, _currentLayout } = this.props;
        const heightRatio = (_currentLayout === LAYOUTS.HORIZONTAL_FILMSTRIP_VIEW ? 1.5 : 1.52);

        return Math.min(_height / heightRatio, _width - 30, 270);
    }
    ...
    _getContainerClassName() {
        ...
        // не показываем рамку на замьюченном спикере, а также если в режим спикера перешли
        if (!_isDominantSpeakerDisabled && _participant?.dominantSpeaker && !_isDominantSpeakerAudioMuted) {
            ...
            <span
                ...
                <div
                    className = {clsx(classes.indicatorsContainer,
                        ...
                        _thumbnailType === THUMBNAIL_TYPE.TILE && classes.indicatorsBottomContainerCenter
                    )}>
                    <ThumbnailBottomIndicatorsMobile
                        className = {_thumbnailType === THUMBNAIL_TYPE.TILE ? classes.indicatorsBackgroundBottomFlexShrink0 : classes.indicatorsBackground}
                        local = {local}
                        participantId = {id}
                ...
                <div
                    className = {clsx(classes.indicatorsContainer,
                        _thumbnailType === THUMBNAIL_TYPE.TILE && classes.indicatorsTopContainerBetween
                    )}>
                    <ThumbnailTopIndicatorsMobile
                        disableConnectionIndicator = {isWhiteboardParticipant(_participant)}
                        indicatorsClassName = {_thumbnailType === THUMBNAIL_TYPE.TILE ? classes.indicatorsBackgroundTopFlexShrink0 : classes.indicatorsBackground}
                        participantId = {id}
                        thumbnailType = {_thumbnailType} />
                </div>
    ...
    render() {
        ...
        if (_isVirtualScreenshareParticipant) {
            ...
            const { _videoTrack, _thumbnailType } = this.props;
            const classes = withStyles.getClasses(this.props);
...
function _mapStateToProps(state: IReduxState, ownProps: any): Object {
    ...
    case THUMBNAIL_TYPE.HORIZONTAL: {
        ...
        _isMobilePortrait = state['features/base/responsive-ui'].aspectRatio === ASPECT_RATIO_NARROW;
    ...
    let isDominantSpeakerAudioMuted = false;
    if (participant?.dominantSpeaker) {

        const sources = participant.sources?.get(MEDIA_TYPE.AUDIO);
        if (sources) {
            isDominantSpeakerAudioMuted = Array.from(sources.values())[0].muted;
        }
    }
    ...
    return {
        ...
        _isDominantSpeakerAudioMuted: isDominantSpeakerAudioMuted,
    };
}

export default connect(_mapStateToProps)(withStyles(translate(ThumbnailMobile), defaultStyles));
