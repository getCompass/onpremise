...
import { getResourceId, getSessionById, registerRecordingAudioFiles, unregisterRecordingAudioFiles } from "./functions";
...
MiddlewareRegistry.register(({ dispatch, getState }) => (next) => async (action) => {
    ...
    switch (action.type) {
        ...
        case START_LOCAL_RECORDING: {
            ...
            try {
                await LocalRecordingManager.startLocalRecording({ dispatch, getState }, action.onlySelf);
                const props = {
                    descriptionKey: "recording.on",
                    titleKey: "dialog.recording",
                };
