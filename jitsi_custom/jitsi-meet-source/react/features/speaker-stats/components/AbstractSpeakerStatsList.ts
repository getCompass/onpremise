...
const abstractSpeakerStatsList = (speakerStatsItem: Function): Function[] => {
    ...
    const userIds = localSortedSpeakerStatsIds.filter(id => localSpeakerStats[id] && !localSpeakerStats[id].hidden && !localSpeakerStats[id].hasLeft());

    return userIds.map((userId, index) => {
        const statsModel = localSpeakerStats[userId];
        const participant = statsModel.isLocalStats() ? localParticipant : remoteParticipants.get(userId);
        const props = {
            isDominantSpeaker: statsModel.isDominantSpeaker(),
            isLocalStats: statsModel.isLocalStats(),
            isModerator: isParticipantModerator(participant),
            ...
            showFaceExpressions,
            isLastItem: index === userIds.length - 1,
