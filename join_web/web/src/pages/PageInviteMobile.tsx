...
import {authInputState, authState, joinLinkState, nameInputState, prepareJoinLinkErrorState} from "../api/_stores.ts";
...
const PageInviteMobile = ({headerContent}: PageInviteMobileProps) => {
	...
	const langStringPageInviteOpenCompassWaitPostModerationMobileTitlePt1 = useLangString("page_invite.open_compass_wait_post_moderation_mobile.title_pt1");
	const langStringPageInviteOpenCompassWaitPostModerationMobileTitlePt2 = useLangString("page_invite.open_compass_wait_post_moderation_mobile.title_pt2");
	const langStringPageInviteOpenCompassWaitPostModerationMobileButton = useLangString("page_invite.open_compass_wait_post_moderation_mobile.button");
	...
	useEffect(() => {
		...
		if (copyButtonRef.current) {
			...
			copyButtonRef.current.addEventListener("touchstart", function () {
				this.style.backgroundColor = "#049a54";
			}, false);

			copyButtonRef.current.addEventListener("touchend", function () {
			...
			<VStack gap="8px" width="100%">
					...
					<VStack p="16px" gap="16px" bgColor="434455" rounded="8px">
						...
						>
							<styled.span
								...
								fontWeight="700"
							>{joinLink === null || joinLink.is_waiting_for_postmoderation === 0 ? langStringPageInviteOpenCompassMobileTitlePt1 : langStringPageInviteOpenCompassWaitPostModerationMobileTitlePt1}
							</styled.span>
							<br/>
							{joinLink === null || joinLink.is_waiting_for_postmoderation === 0 ? langStringPageInviteOpenCompassMobileTitlePt2 : langStringPageInviteOpenCompassWaitPostModerationMobileTitlePt2}
						</Text>
						<Button
							px="16px"
							size="full"
							...
							onClick={() => {
								...
								if (joinLink !== null) {
									...
									window.location.replace(`getcompassonpremise://spaceJoin?spaceId=${joinLink.company_id}&inviterUserId=${joinLink.inviter_user_id}&isPostModeration=${joinLink.is_postmoderation === 1}&role=${joinLink.role}&isPreviousSpaceMember=${joinLink.was_member_before === 1}`)
									return;
								}

								if (prepareJoinLinkError !== null && prepareJoinLinkError.error_code === ALREADY_MEMBER_ERROR_CODE && prepareJoinLinkError.data !== undefined) {

									const prepareJoinLinkErrorData = prepareJoinLinkError.data as PrepareJoinLinkErrorAlreadyMemberData;
									window.location.replace(`getcompassonpremise://spaceJoin?spaceId=${prepareJoinLinkErrorData.company_id}&inviterUserId=${prepareJoinLinkErrorData.inviter_user_id}&isPostModeration=${prepareJoinLinkErrorData.is_postmoderation === 1}&role=${prepareJoinLinkErrorData.role}&isPreviousSpaceMember=${prepareJoinLinkErrorData.was_member_before === 1}`)
									return;
								}
							}}
						>
							{joinLink === null || joinLink.is_waiting_for_postmoderation === 0 ? langStringPageInviteOpenCompassMobileButton : langStringPageInviteOpenCompassWaitPostModerationMobileButton}
						</Button>
					</VStack>
				</Box>
				...
				<Box w="100%" px="24px">
					<VStack p="16px" gap="16px" bgColor="434455" rounded="8px">
						...
						{apiAuthGenerateToken.isLoading || !apiAuthGenerateToken.data || (apiJoinLinkAccept.isLoading) ? (
							<VStack
								w="100%"
								bgColor="000000.01"
								rounded="8px"
							...
							<Box
								...
								onClick={() => {
									...
									if (tokenBoxRef.current && scrollableParentBlockRef.current) {
										copyToClipboard(apiAuthGenerateToken.data.authentication_token, scrollableParentBlockRef.current, tokenBoxRef.current)
									}
								}}
							>
								<Text
									...
									fs="16"
									lh="22"
								>{apiAuthGenerateToken.data.authentication_token.substring(0, 80)}</Text>
							</Box>
						)}
						<Button
							ref={copyButtonRef}
							...
							onClick={() => {
								...
								if (!apiAuthGenerateToken.data || !scrollableParentBlockRef.current || !tokenBoxRef.current) {
									return;
								}
