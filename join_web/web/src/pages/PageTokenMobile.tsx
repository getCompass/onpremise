...
import { MOBILE_PLATFORM_ANDROID, MOBILE_PLATFORM_HUAWEI, MOBILE_PLATFORM_IOS } from "../api/_types.ts";
...
const StepTwoContent = ({
	...
	const langStringDownloadCompassMobileStoresAppstore = useLangString("download_compass.mobile_stores.appstore");
	const langStringDownloadCompassMobileStoresGooglePlay = useLangString("download_compass.mobile_stores.google_play");
	const langStringDownloadCompassMobileStoresAppGallery = useLangString("download_compass.mobile_stores.app_gallery");
	...
	const onClickHandler = useCallback(() => {
		...
		setStoreMenuOpen(true);
	}, [ deviceDetect, apiAuthGenerateToken ]);
	...
	}, [ onClickHandler, joinLink, loginType, apiAuthGenerateToken, scrollableParentBlockRef.current, timeLeft ]);
				...
				<VStack w = "100%" gap = "0px">
					...
					<Button
						...
						onClick = {(event) => {
							...
							onInstallClickHandler().then(() => onClickHandler());
						}}
					>
							...
							<MenuItemGroup id = "stores">
								<MenuItem id = {MOBILE_PLATFORM_IOS}>
									<HStack gap = "12px">
										...
										<Text fs = "18" lh = "27" color = "333e49" opacity = "80%" font = "regular">
											{langStringDownloadCompassMobileStoresAppstore}
										</Text>
									</HStack>
								</MenuItem>
								<MenuItem id = {MOBILE_PLATFORM_ANDROID}>
									<HStack gap = "12px">
										<GooglePlayIcon />
										<Text fs = "18" lh = "27" color = "333e49" opacity = "80%" font = "regular">
											{langStringDownloadCompassMobileStoresGooglePlay}
										</Text>
									</HStack>
								</MenuItem>
								<MenuItem id = {MOBILE_PLATFORM_HUAWEI}>
									<HStack gap = "12px">
										<AppGalleryIcon />
										<Text fs = "18" lh = "27" color = "333e49" opacity = "80%" font = "regular">
											{langStringDownloadCompassMobileStoresAppGallery}
										</Text>
									</HStack>
								</MenuItem>
							</MenuItemGroup>
...
const StepOneContent = ({ scrollableParentBlockRef, parentBlockRef, setStoreMenuOpen }: StepOneContentProps) => {
	...
	const onRegistrationClickHandler = useCallback(() => {
		...
		return;
	}, [ apiAuthGenerateToken, scrollableParentBlockRef.current, parentBlockRef.current ]);

	const onLoginClickHandler = useCallback((event: React.MouseEvent) => {

		if (!apiAuthGenerateToken.data) {
			return;
		}
		...
		setStoreMenuOpen(true);
	}, [ joinLink, apiAuthGenerateToken, scrollableParentBlockRef.current ]);
	...
	return (
		<VStack gap = "16px">
			<HStack gap = "12px">
				...
				<Text color = "white" fs = "16" lh = "22" font = "regular">
					...
					{isRegistration
						...
						: langStringPageTokenStep1LoginDescPt2Mobile}
				</Text>
			</HStack>
				...
				<VStack w = "100%" rounded = "8px" gap = "0px" alignItems = "start">
					<Box ref = {parentBlockRef} w = "100%" bgColor = "000000.01" rounded = "8px" px = "8px" py = "12px">
						<Text
							...
							font = "regular"
							onClick = {() => {
								if (parentBlockRef.current && !isAuthenticationTokenExpired) {
									copyToClipboard(
										apiAuthGenerateToken.data.authentication_token,
										scrollableParentBlockRef.current,
										parentBlockRef.current
			...
			<Button
				...
				size = "full"
				onClick = {(event: React.MouseEvent) => isRegistration ? onRegistrationClickHandler() : onLoginClickHandler(event)}
			>
				{apiAuthGenerateToken.isLoading || !apiAuthGenerateToken.data ? (
					<Preloader20 />
				) : isRegistration ? (
					...
					<VStack w = "100%">
						<VStack w = "100%" gap = "24px">
							...
							<VStack w = "100%" alignItems = "center" gap = "4px">
								<Text w = "100%" textAlign = "center" fs = "20" lh = "28" color = "white" font = "bold"
									  ls = "-03">
									{langStringPageTokenTitle}
								</Text>
								<Text w = "100%" textAlign = "center" fs = "16" lh = "22" color = "white"
									  font = "regular">
