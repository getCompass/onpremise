...
const CreateProfileDialogContentDesktop = () => {
	...
	const langStringCreateProfileDialogConfirmCancelDesktopTitle = useLangString(
		"create_profile_dialog.confirm_cancel_desktop.title"
	);
	const langStringCreateProfileDialogConfirmCancelDesktopDesc = useLangString(
		"create_profile_dialog.confirm_cancel_desktop.desc"
	);
	const langStringCreateProfileDialogConfirmCancelDesktopCancelButton = useLangString(
		"create_profile_dialog.confirm_cancel_desktop.cancel_button"
	);
	const langStringCreateProfileDialogConfirmCancelDesktopConfirmButton = useLangString(
		"create_profile_dialog.confirm_cancel_desktop.confirm_button"
	);
	const langStringCreateProfileDialogIncorrectNameTooltip = useLangString(
		"create_profile_dialog.incorrect_name_tooltip"
	);
	const langStringCreateProfileDialogNotSavedSymbolsTooltip = useLangString(
		"create_profile_dialog.not_saved_symbols_tooltip"
	);
	...
	const userInfo = useAtomValue(userInfoDataState);
	...
	const name = useMemo(() => {
		...
		// если ранее пытались ввести имя и оно есть в кэше, то используем его в первую очередь
		if (parseInt(expiresAt) > dayjs().unix()) {
			return name;
		}

		// если есть имя с сервера
		if (userInfo !== null && userInfo.full_name !== "") {
	...
	return (
		<VStack w="100%" gap="22px">
			<VStack px="6px" mt="20px" gap="16px" w="100%">
				<CreateProfileIcon80 />
				<VStack gap="6px" w="100%">
					<Text fs="18" lh="24" font="bold" ls="-02">
						{langStringCreateProfileDialogTitle}
					</Text>
					<Text fs="14" lh="20" textAlign="center" font="regular">
						{langStringCreateProfileDialogDesc}
					</Text>
				</VStack>
			</VStack>
			<VStack w="100%" gap="22px">
				...
				<HStack w="100%" justify="space-between">
					<Dialog style="desktop" position="start" size="medium" backdrop="opacity80">
						<DialogTrigger asChild>
							<Button size="px16py6" color="f5f5f5" textSize="xl_desktop">
								{langStringCreateProfileDialogCancelButton}
							</Button>
						</DialogTrigger>
						<Portal>
							<DialogBackdrop />
							<DialogContainer>
								<DialogContent overflow="hidden" lazyMount unmountOnExit>
									<VStack w="100%" gap="8px">
										...
										<HStack pt="16px" w="100%" justify="space-between">
											...
											>
												<HStack gap="4px">
													<Box>
														{langStringCreateProfileDialogConfirmCancelDesktopConfirmButton}
													</Box>
													<Box w="20px" h="21px">
														<svg
															width="20"
...
const CreateProfileDialogContentMobile = () => {
	...
	const langStringCreateProfileDialogCancelButton = useLangString("create_profile_dialog.cancel_button");
	const langStringCreateProfileDialogConfirmButton = useLangString("create_profile_dialog.confirm_button");
	const langStringCreateProfileDialogConfirmCancelMobileTitle = useLangString(
		"create_profile_dialog.confirm_cancel_mobile.title"
	);
	const langStringCreateProfileDialogConfirmCancelMobileShortTitle = useLangString(
		"create_profile_dialog.confirm_cancel_mobile.short_title"
	);
	const langStringCreateProfileDialogConfirmCancelMobileConfirmButton = useLangString(
		"create_profile_dialog.confirm_cancel_mobile.confirm_button"
	);
	const langStringCreateProfileDialogConfirmCancelMobileCancelButton = useLangString(
		"create_profile_dialog.confirm_cancel_mobile.cancel_button"
	);
	const langStringCreateProfileDialogIncorrectNameTooltip = useLangString(
		"create_profile_dialog.incorrect_name_tooltip"
	);
	...
	return (
		<VStack w="100%" gap="24px">
			...
			<VStack w="100%" gap="24px">
				...
				{isCanceling ? (
					<HStack w="100%" justify="space-between">
						<Text font="bold" ls="-03" fs="16" lh="20" color="333e49" ref={cancelTextRef}>
							{cancelTextWidth < 190
								? langStringCreateProfileDialogConfirmCancelMobileShortTitle
								: langStringCreateProfileDialogConfirmCancelMobileTitle}
						</Text>
						<HStack gap="12px">
							<Button
								size="px16py9"
								...
								disabled={apiAuthLogout.isLoading}
							>
								{langStringCreateProfileDialogConfirmCancelMobileConfirmButton}
							</Button>
							<Button size="px16py9" color="f5f5f5" onClick={() => setIsCanceling(false)}>
								{langStringCreateProfileDialogConfirmCancelMobileCancelButton}
							</Button>
						</HStack>
					</HStack>
				) : (
					<HStack w="100%" justify="space-between">
						<Button size="px16py9" color="f5f5f5" onClick={() => setIsCanceling(true)}>
							{langStringCreateProfileDialogCancelButton}
						</Button>
						<Button
							size="px16py9"
