 ...
 */
class Integration_Handler extends Api implements \RouteHandler {
	...
	public function handle(string $route, array $post_data, int $user_id = 0):array {
		...
		$extra = [
			...
			"authorization_token" => INTEGRATION_AUTHORIZATION_TOKEN,
		];

		// здесь описана последовательность проверок
		$router = new \BaseFrame\Router\Middleware([
			\BaseFrame\Router\Middleware\IntegrationEnabled::class,
			\BaseFrame\Router\Middleware\ValidateRequest::class,
			\BaseFrame\Router\Middleware\IntegrationAuthorization::class,
