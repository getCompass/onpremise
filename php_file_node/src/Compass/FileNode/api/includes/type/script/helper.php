...
class Type_Script_Helper {
	...
	// пример pattern: #^dpc_thread_\d{4}_\d{1,2}$#si
	...
	protected static function _isDbExist(string $sharding_key):bool {
		...
		try {
			...
			throw new \ReturnException("Got not 1049 error_code from MySQL in " . __METHOD__ . "\n{$e->getMessage()}");
		}
