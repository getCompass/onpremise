...
mysql --user="${MYSQL_USER}" --password="${MYSQL_PASS}" --host="$MYSQL_HOST" -P "$MYSQL_SYSTEM_PORT" < "${SCRIPT_PATH}/sql/init_node.sql"
...
mysql --user="${MYSQL_USER}" --password="${MYSQL_PASS}" --host="$MYSQL_HOST" -P "$MYSQL_SYSTEM_PORT" < "${SCRIPT_PATH}/sql/init_system.sql"
