...
class Script_Update_Release45_Example {
	...
	public static function doWork(string $sharding_key, string $table_key):void {
		...
		do {
			...
			$list  = \sharding::pdo($sharding_key)->getAll($query, $table_key, 1, self::_QUERY_LIMIT, $offset);
...
// получаем список баз
$database_list = Type_Script_Helper::getDatabaseList("#^dpc_thread_\d{4}_\d{1,2}$#si");
