...
use Compass\Pivot\Gateway_Notice_Sender;
...
class Monitoring {
	...
	protected static function _prepareText(string $text):string {
		...
		// header
		$header = ":warning: *Exception* on *" . SERVER_NAME . "* server, module:" . SERVICE_NAME . " :warning:\n";
	...
	protected static function _send(array $logs):void {

		if (NOTICE_GROUP_KEY_ONPREMISE === "") {
			return;
		}

		$text = implode("\n", $logs);
		Gateway_Notice_Sender::sendGroup(NOTICE_ENDPOINT_COMPASS_ONPREMISE, NOTICE_BOT_TOKEN_ONPREMISE, NOTICE_GROUP_KEY_ONPREMISE, mb_substr($text, 0, 1500));
	}
