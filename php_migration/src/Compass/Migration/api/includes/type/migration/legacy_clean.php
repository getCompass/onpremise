...
/**
 * класс для работы миграций по очистке легаси
 */
class Type_Migration_LegacyClean {
		...
		foreach ($database_list as $database) {
			...
			foreach ($full_database_name_list as $full_database_name) {
				...
				try {
					...
					$row = Gateway_Db_CompanySystem_MigrationCleaningDatabaseList::get($full_database_name);
				} catch (cs_RowIsEmpty) {

					// если записи нет то вставляем
					$row = Gateway_Db_CompanySystem_MigrationCleaningDatabaseList::insert(
						$full_database_name,
						$database["name"],
						1,
						0,
				...
				$migration_path = Type_Migration_Main::getMigrationPath($database["name"], Type_Migration_Main::MIGRATION_LEGACY_CLEAN_PATH);
	...
	protected static function _doDatabase(Struct_Db_CompanySystem_MigrationDatabase $row, string $migration_path, int|false $version):void {
		...
		$migration_sql_legacy_clean_path = "start";

		$release_row = Gateway_Db_CompanySystem_MigrationReleaseDatabaseList::get($row->full_database_name);

		// дополнительная проверка
		while (mb_strlen($migration_sql_legacy_clean_path) > 0) {
			...
			$migration_sql_legacy_clean_path = self::_getMigrationFilePath($expected_version, $migration_path);

			// если миграции не нашли то и накатывать нечего :)
			if (mb_strlen($migration_sql_legacy_clean_path) < 1) {

				Type_Migration_Check::do($row->current_version, $row->database_name, $migration_path, $row->full_database_name);
				return;
			}

			// проверяем что миграцию можно накатить на текущую базу
			Type_Migration_LegacyCleanCheck::do($expected_version, $migration_path, $release_row->current_version);

			// когда дошли сюда значит все готово к миграции и нам остатеся только накатить. Блокируем бд
			$set = [
				"is_completed"        => 0,
				"expected_version"    => $expected_version,
				"last_migrated_type" => Type_Migration_Main::MIGRATION_TYPE_LEGACY_CLEAN,
				"last_migrated_at"   => time(),
				"last_migrated_file" => $migration_sql_legacy_clean_path,
			];
			Gateway_Db_CompanySystem_MigrationCleaningDatabaseList::set($row->full_database_name, $set);

			$sql = file_get_contents($migration_sql_legacy_clean_path);
