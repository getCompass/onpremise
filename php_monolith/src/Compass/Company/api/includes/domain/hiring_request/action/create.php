...
class Domain_HiringRequest_Action_Create {
	 ...
	 * @long
	 */
	public static function do(int $hired_by_user_id, int $invited_user_id, string $invite_link_uniq, int $entry_id, int $entry_option,
					  string $comment, string $locale):int {
		...
		if ($entry_option === Domain_JoinLink_Entity_Main::ENTRY_OPTION_NEED_POSTMODERATION) {
			...
			$extra        = new Domain_Member_Entity_Notification_Extra(
				...
				$hiring_request->extra["extra"]["candidate_user_info"]["full_name"],
				$company_name
			);
