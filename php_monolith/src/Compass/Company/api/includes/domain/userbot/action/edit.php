...
class Domain_Userbot_Action_Edit {
	...
	public static function do(Struct_Db_CloudCompany_Userbot $userbot,
		...
		self::_editProfileInfo($userbot->user_id, $short_description, $userbot_name, $avatar_color_id);

		// редактируем данные бота
		$userbot = self::_editUserbotInfo($userbot, $avatar_color_id, $is_react_command, $webhook);

		// редактируем бота на пивоте, если необходимо
		if ($userbot_name !== false || $avatar_color_id !== false || $is_react_command !== false || $webhook !== false) {

			$token = Domain_Userbot_Entity_Userbot::getToken($userbot->extra);
			Gateway_Socket_Pivot::editUserbot($userbot->userbot_id, $token, $userbot_name, $avatar_color_id, $is_react_command, $webhook);
		}
	 ...
	 * @throws \parseException
	 */
	protected static function _editProfileInfo(int $userbot_user_id, string|false $short_description, string|false $userbot_name, int|false $avatar_color_id):void {
		...
		if ($userbot_name === false && $avatar_color_id === false) {
	 ...
	 * @throws \parseException
	 */
	protected static function _editUserbotInfo(Struct_Db_CloudCompany_Userbot $userbot,
