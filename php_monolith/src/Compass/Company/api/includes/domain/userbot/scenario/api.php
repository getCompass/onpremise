...
class Domain_Userbot_Scenario_Api {
	...
	public static function getUserRel(array $batch_user_list):array {
		...
		foreach ($batch_user_list as $user_list) {
			...
			if (!isset($user_list["user_id_list"]) || !isset($user_list["signature"])) {
				throw new cs_WrongSignature("not found user_id_list or signature");
			}
			...
			// проверяем подпись
			if (!Domain_Member_Entity_Validator::verifyUserListSignature($user_list["user_id_list"], $user_list["signature"])) {
				throw new cs_WrongSignature("incorrect user_id_list or signature");
			}
		}
