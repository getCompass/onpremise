...
/**
 * Задача очистки диалога для всех пользователей.
 * Вызывается при полной очистке диалога.
 */
class Domain_Search_Entity_Conversation_Task_Purge extends Domain_Search_Entity_Task {

	/** @var int тип задачи */
	public const TASK_TYPE = Domain_Search_Const::TASK_TYPE_CONVERSATION_PURGE;
	 ...
	 */
	public static function queue(string $conversation_map):void {

		// добавляем задачи в список
		$task_data   = new Struct_Domain_Search_IndexTask_Entity($conversation_map);
	...
	public static function execList(array $task_list):void {
		...
		foreach ($task_data_list as $task_data) {
			...
			if (!isset($entity_search_rel_list[$task_data->entity_map])) {
				...
				static::yell("[WARN] search rel для диалога %s не найдена, пропускаю", $task_data->entity_map);
				continue;
			}

			$search_id = $entity_search_rel_list[$task_data->entity_map];
			Gateway_Search_Main::deleteByParent($search_id);

			static::say("удалил данные диалога для всех пользователей");
		}
	}
}
