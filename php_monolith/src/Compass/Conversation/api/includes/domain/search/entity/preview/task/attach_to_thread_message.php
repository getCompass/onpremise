...
/**
 * Задача прикрепления превью к сообщению-комментарию.
 *
 * Запускается при:
 * 	— Успешном парсинге превью
 * 	— Переиндексации сообщения
 */
class Domain_Search_Entity_Preview_Task_AttachToThreadMessage extends Domain_Search_Entity_Task {

	/** @var int тип задачи */
	public const TASK_TYPE = Domain_Search_Const::TASK_TYPE_THREAD_PREVIEW_INDEX;
	...
	public static function _preload(array $task_data_list):void {
		...
		foreach ($task_data_list as $task_data) {
			...
			$to_resolve_entity_search_id_list[] = new Struct_Domain_Search_AppEntity(Domain_Search_Const::TYPE_PREVIEW, $task_data->preview_map);
	...
	protected static function _resolveParentData(Struct_Domain_Search_IndexTask_MessagePreview $task_data):array|false {
		...
		if (!isset($parent_conversation_search_id, $parent_message_search_id, $thread_search_id, $thread_message_search_id)) {
			...
			static::yell("[WARN] search rel для одной из сущностей (%s, %s, %s, %s) не найдена",
