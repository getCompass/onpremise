...
/**
 * Класс повторной индексации сообщений.
 * Вызывается при изменении сущности, после первичной индексации.
 */
class Domain_Search_Entity_ThreadMessage_Task_Reindex extends Domain_Search_Entity_Task {

	/** @var int тип задачи */
	public const TASK_TYPE = Domain_Search_Const::TASK_TYPE_THREAD_MESSAGE_REINDEX;
	...
	public static function execList(array $task_list):void {
		...
		foreach ($replace_list as $replace) {
			...
			// удаляем привязанные превью и файлы, превью будут перепаршены
	...
	protected static function _preload(array $task_data_list):void {
		...
		foreach ($task_data_list as $task_data) {
			...
			$to_resolve_search_id_message_list[] = new Struct_Domain_Search_AppEntity(Domain_Search_Const::TYPE_THREAD_MESSAGE, $task_data->entity_map);
		}
		...
		Domain_Search_Repository_ProxyCache_ThreadMessage::load($message_map_list);
	 ...
	 */
	protected static function _toIndexData(Struct_Domain_Search_IndexTask_LocalizedEntity $task_data):Struct_Domain_Search_Replace|false {
