 ...
 */
class Domain_Oidc_Scenario_Endpoint_Oidc {
	...
	public static function onReceiveAuthorizationCode(string $code, string $received_state):string {
		...
		// распаковываем state
		try {
			$state = new Domain_Oidc_Entity_State($received_state);
		} catch (Domain_Oidc_Exception_Auth_State_Invalid) {
		...
		try {
			$sso_auth = Domain_Oidc_Entity_Auth::get($state->getSsoAuthToken());
		} catch (Domain_Oidc_Exception_Auth_TokenNotFound) {
			throw new EndpointAccessDeniedException("unexpected behaviour");
		}

		// если попытка уже протухла
		if (Domain_Oidc_Entity_Auth::isExpired($sso_auth)) {
			throw new EndpointAccessDeniedException("auth attempt is expired");
		}

		// если попытка уже завершена
		if (Domain_Oidc_Entity_Auth::isSsoAuthComplete($sso_auth) || Domain_Oidc_Entity_Auth::isCompassAuthComplete($sso_auth)) {
			throw new EndpointAccessDeniedException("auth attempt is complete");
		}

		// запрашиваем токены и сохраняем их
		Domain_Oidc_Action_Auth_Oidc_RequestTokens::do($sso_auth, $code, $received_state);
