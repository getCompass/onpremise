 ...
 */
class Domain_Sso_Entity_Oidc_Token {
	 ...
	 */
	public static function save(Struct_Db_SsoData_SsoAuth $sso_auth, Struct_Sso_Oidc_TokenResponse $token_response):void {
	...
	public static function getIdTokenCustomFieldValue(string $id_token, string $field_name):string {
		...
		if (!isset($payload_map[$field_name])) {
			...
			Domain_Sso_Entity_Logger::log("id_token не содержит ожидаемое поле $field_name");
			return "";
		}
	...
	public static function getSub(string $id_token):string {
		...
		if (!isset($payload_map["sub"])) {
			...
			Domain_Sso_Entity_Logger::log("id_token payload не содержит поле sub");
