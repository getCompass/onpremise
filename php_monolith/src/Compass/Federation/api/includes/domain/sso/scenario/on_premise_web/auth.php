 ...
 */
class Domain_Sso_Scenario_OnPremiseWeb_Auth {
	...
	public static function begin(string|bool $redirect_url):Struct_Db_SsoData_SsoAuth {
		...
		// запускаем попытку
		return Domain_Sso_Action_Auth_Start::do(UserAgent::getUserAgent(), getIp(), $redirect_url);
	}
	...
	public static function getStatus(string $sso_auth_token, string $signature):string {
		...
		$auth = Domain_Sso_Entity_Auth::get($sso_auth_token);

		// проверяем, что подпись верна
		Domain_Sso_Entity_Auth::assertSignature($auth, $signature);
	...
	protected static function _resolveStatus(Struct_Db_SsoData_SsoAuth $auth):string {
		...
		if (Domain_Sso_Entity_Auth::isCompassAuthComplete($auth)) {
			return self::STATUS_COMPLETED;
		}

		// если попытка протухла
		if (Domain_Sso_Entity_Auth::isExpired($auth)) {
			return self::STATUS_EXPIRED;
		}

		// если попытка готова к продолжению аутентификации
		if (Domain_Sso_Entity_Auth::isSsoAuthComplete($auth)) {
