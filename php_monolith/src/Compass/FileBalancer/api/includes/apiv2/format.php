...
class Apiv2_Format
{
	...
	public static function file(array $file_row): array
	{
		...
		// если файл удален, отдаем пустой
		if ($file_row["is_deleted"] || $file_row["status"] !== Type_File_Main::FILE_STATUS_APPROVED) {
			return self::_formatDeletedFile($file_row);
		}
		...
		$output = [
			...
			"type"           => (string) Type_File_Main::FILE_TYPE_NAME[$file_row["type"]],
	...
	/**
	 * Отформатировать удаленный файл
	 */
	protected static function _formatDeletedFile(array $file_row): array
	{
	...
	/**
	 * Отформатировать waveform в голосовых аудио
	 */
	...
	/**
	 * Основная информация об оригинальном видео
	 *
	 *
	 * @return int[]
	 */
