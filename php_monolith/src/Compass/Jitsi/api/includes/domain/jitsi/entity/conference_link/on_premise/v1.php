...
/**
 * класс для работы с ссылками на конференцию на on-premise окружении
 * @package Compass\Jitsi
 */
class Domain_Jitsi_Entity_ConferenceLink_OnPremise_V1 implements Domain_Jitsi_Entity_ConferenceLink_Interface_LinkHandler {

	/** @var int длинна пароля */
	...
	public static function prepareLandingConferenceLink(Struct_Db_JitsiData_Conference $conference):string {
		...
		return sprintf("%s://%s/c/%s?pwd=%d%s", WEB_PROTOCOL_PUBLIC, DOMAIN_JITSI, $unique_part, $conference->creator_user_id, $conference->password);
	}
	...
	public static function prepareJitsiConferenceLink(Struct_Db_JitsiData_Conference $conference, Struct_Jitsi_Node_Config $node_config, string $jwt_token):string {
		...
		return sprintf("%s://%s%s/%s?jwt=%s", WEB_PROTOCOL_PUBLIC, $conference->jitsi_instance_domain, $node_config->subdir, $conference->conference_id, $jwt_token);
	}
