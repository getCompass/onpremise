...
class Type_Task_Handler {
	...
	protected array $_listener_class_list = [
		Domain_PhpJitsi_Scenario_Event::class,
	];
	...
	public function handle(array $task):Type_Task_Struct_Response {
		...
		if (!isset($task["name"])) {
			throw new \paramException("task name not set");
		}
		...
		if (!isset($this->_executor_list[$task["name"]])) {
			...
			return Type_Task_Struct_Response::build(static::DELIVERY_STATUS_ERROR);
		}
	 ...
	 * @return array
	 */
	#[\JetBrains\PhpStorm\ArrayShape(["result_state" => "int", "processed_in" => "int", "need_work_at" => "int", "message" => "string"])]
		...
		foreach ($this->_listener_class_list as $listener_class) {
			...
			foreach ($reflection_class->getMethods() as $method_reflection) {
				...
				if (count($attribute_reflection_list) > 1) {
					throw new \parseException("task has more than one executor");
				}
				...
				if ($method_reflection->getReturnType()?->getName() !== Type_Task_Struct_Response::class) {
					throw new \parseException("passed task executor returns incorrect type, " . Type_Task_Struct_Response::class . " expected");
				}
