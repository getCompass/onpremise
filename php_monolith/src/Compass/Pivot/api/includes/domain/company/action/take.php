...
class Domain_Company_Action_Take {
	...
	protected const _HIBERNATION_IMMUNITY_TILL = DAY14;
	 ...
	 *
	 * @param Struct_Db_PivotCompany_Company $company_to_take
	 * @param int                            $created_by_user_id
	 * @param int                            $creator_partner_id
	 * @param int                            $avatar_color_id
	 * @param string                         $name
	 * @param string                         $client_company_id
	 * @param string                         $avatar_file_map
	...
	protected static function _joinCreator(Struct_Db_PivotCompany_Company $company, bool $is_need_create_intercom_conversation):array {
		...
		try {
			...
			[$user_company, $company] = Domain_Company_Action_Member_AddCreator::do(
				$company->created_by_user_id, $company, Locale::getLocale(), $is_need_create_intercom_conversation
			);
		} catch (\Exception $e) {

			// случай если не получилось добавить создателя
			Type_System_Admin::log("company_add_owner",
				"Не смогли добавить пользователя {$company->created_by_user_id} в компанию {$company->company_id}", true);
