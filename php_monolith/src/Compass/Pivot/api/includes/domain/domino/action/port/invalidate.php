...
class Domain_Domino_Action_Port_Invalidate {
	 ...
	 */
	public static function run(Struct_Db_PivotCompanyService_DominoRegistry $domino, int $port, string $invalidate_reason = ""):void {
		...
		try {
			...
			Gateway_Db_PivotCompanyService_PortRegistry::getForUpdate($domino->domino_id, $port);

			// обновляем запись для порта, устанавливая время блокировки
			Gateway_Db_PivotCompanyService_PortRegistry::set($domino->domino_id, $port, [
		...
		try {
			...
			Gateway_Bus_DatabaseController::invalidatePort($domino, $port);
		} catch (\Exception) {
