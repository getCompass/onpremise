...
/**
 * Действие привязки порта к компании.
 * Выполняет блокирование порта для дальнейшей передачи компании.
 */
class Domain_Domino_Action_Port_LockForCompany {
	 ...
	 */
	public static function run(Struct_Db_PivotCompanyService_DominoRegistry $domino, int $company_id, int $port_type, int $lock_duration):Struct_Db_PivotCompanyService_PortRegistry {

		// получаем уже заблокированный обновленный порт
		$void_port = static::_getLocked($domino, $company_id, $port_type, $lock_duration);
	 ...
	 */
	protected static function _getLocked(Struct_Db_PivotCompanyService_DominoRegistry $domino, int $company_id, int $port_type, int $lock_duration):Struct_Db_PivotCompanyService_PortRegistry {

		try {

			return static::_getExisting($domino, $company_id, $port_type);
		...
		try {
			...
			$void_port = Gateway_Db_PivotCompanyService_PortRegistry::getByCriteriaForUpdate(
				$domino->domino_id, $port_type, Domain_Domino_Entity_Port_Registry::STATUS_VOID
			);
		} catch (\BaseFrame\Exception\Gateway\RowNotFoundException) {

			Gateway_Db_PivotCompanyService_PortRegistry::rollback();
			throw new Domain_Domino_Exception_VoidPortsExhausted("there is no void port with type {$port_type}");
		}
	 ...
	 * @throws \returnException
	 */
	protected static function _getExisting(Struct_Db_PivotCompanyService_DominoRegistry $domino, int $company_id, int $port_type):Struct_Db_PivotCompanyService_PortRegistry {
		...
		try {
			...
			$existing_port = Gateway_Db_PivotCompanyService_PortRegistry::getForResolveForCompany(
				$domino->domino_id, $port_type, Domain_Domino_Entity_Port_Registry::STATUS_LOCKED, $company_id, time() + HOUR24
			);
