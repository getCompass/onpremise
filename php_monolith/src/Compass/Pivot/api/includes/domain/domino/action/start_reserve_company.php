 ...
 */
class Domain_Domino_Action_StartReserveCompany
{
	...
	public static function run(Struct_Db_PivotCompanyService_DominoRegistry $domino, Struct_Db_PivotCompany_Company $company): array
	{
		...
		$port_row    = Gateway_Db_PivotCompanyService_PortRegistry::getActiveByCompanyId($domino->domino_id, $company->company_id);
		$target_port = Domain_Domino_Action_Port_LockForReserveCompany::run($domino, $company->company_id, static::_PORT_LOCK_DURATION, $port_row->port, $port_row->host);
		Type_System_Admin::log("start_company_process", "Выбрали порт {$target_port->port} для привязки компании {$company->company_id}");
		Domain_Domino_Action_Port_ReserveBind::run($domino, $target_port, $company->company_id, Domain_Domino_Action_Port_Bind::POLICY_WAKING_UP);
