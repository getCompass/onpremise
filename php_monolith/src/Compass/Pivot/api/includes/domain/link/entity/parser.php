 ...
 */
class Domain_Link_Entity_Parser {
	...
	/** @noinspection HttpUrlsUsage список доступных протоколов */
	...
	protected static function _removeEmojiAndStringAfterFromPath(string $path):string {
		...
		// получаем все эмодзи и флаги
		$flags  = array_values(\BaseFrame\Conf\Emoji::EMOJI_FLAG_LIST);
		$emojis = array_values(\BaseFrame\Conf\Emoji::EMOJI_LIST);

		foreach ($emojis_from_path_match[0] as $emoji_from_path) {

			// проверяем, является ли строка эмодзи
			if (in_array($emoji_from_path, $emojis) || in_array($emoji_from_path, $flags)) {
	...
	/**
	 * убираем пробелы у ссылки
	 *
	 * @param string $link
	 *
	 * @return string
	 */
	public static function trimLink(string $link):string {
