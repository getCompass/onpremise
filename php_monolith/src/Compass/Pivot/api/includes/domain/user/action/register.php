 ...
 *
 * Class Domain_User_Action_Register
 */
class Domain_User_Action_Register {
	 ...
	 */
	public static function do(string $phone_number, int $user_id):Struct_User_Auth_Story {

		$sms_code = self::_getSmsCode($phone_number);

		$config_text = getConfig("LOCALE_TEXT");

		// формируем текст сообщения
		$sms_text = \BaseFrame\System\Locale::getText($config_text, "sms_confirm", "auth", \BaseFrame\System\Locale::getLocale(), [
		...
		$auth_story_data = Domain_User_Entity_StoryHandler_RegisterStory::handle($phone_number, $sms_code, $sms_id);
