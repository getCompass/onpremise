 ...
 *
 * Class Domain_User_Entity_TwoFa_TwoFa
 */
class Domain_User_Entity_TwoFa_TwoFa {

	public const TWO_FA_CHANGE_PIN_TYPE     = 1; // тип изменения пинкода
	public const TWO_FA_SELF_DISMISSAL_TYPE = 2; // тип покидание компании (самоувольнение)
	public const TWO_FA_DELETE_COMPANY      = 3; // тип удаления компании
	public const TWO_FA_DELETE_PROFILE      = 4; // тип удаления аккаунта пользователя

	public const EXPIRE_AT = 60 * 20; // через сколько истекает попытка 2fa действия

	protected const _GROUP_NAME_BY_TYPE = [
		self::TWO_FA_CHANGE_PIN_TYPE     => "auth",
		self::TWO_FA_SELF_DISMISSAL_TYPE => "self_dismissal",
		self::TWO_FA_DELETE_COMPANY      => "delete_company",
		self::TWO_FA_DELETE_PROFILE      => "delete_profile",
	];

	protected const _ACTION_TYPES = [
		self::TWO_FA_CHANGE_PIN_TYPE,
		self::TWO_FA_SELF_DISMISSAL_TYPE,
		self::TWO_FA_DELETE_COMPANY,
		self::TWO_FA_DELETE_PROFILE,
	];

	protected Struct_Db_PivotAuth_TwoFa $two_fa;

	/**
	 * Domain_User_Entity_TwoFa_TwoFa constructor.
	 *
	 */
