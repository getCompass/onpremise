...
class Domain_User_Scenario_Api_AuthV2 {
	...
	public static function sendAttributionOfRegistration(int $user_id, string $session_uniq, int $timezone_utc_offset, int $screen_avail_width, int $screen_avail_height):array {
		...
		// если нашли 100% совпадение с join-страницей
		if ($result->join_page_matched_percentage == 100 && !is_null($result->matched_visit)) {
			return self::_prepareOpenJoinLinkOutput($user_id, $session_uniq, $result->matched_visit);
		}

		// если нашли 1-99% совпадение с join-страницей
		if ($result->join_page_matched_percentage > 0 && $result->join_page_matched_percentage < 100) {
			return ["action" => (string) self::ATTRIBUTION_ACTION_OPEN_ENTERING_LINK, "data" => (object) []];
		}

		// во всех остальных случаях возвращаем по дефолту
		return ["action" => (string) self::ATTRIBUTION_ACTION_OPEN_DASHBOARD, "data" => (object) []];
	}
