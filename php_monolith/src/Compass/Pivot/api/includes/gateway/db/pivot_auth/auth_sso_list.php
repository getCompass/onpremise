...
/**
 * класс-интерфейс для таблицы pivot_auth_{Y}.auth_sso_list_{m}
 */
class Gateway_Db_PivotAuth_AuthSsoList extends Gateway_Db_PivotAuth_Main {

	protected const _TABLE_KEY = "auth_sso_list";
	 ...
	 */
	public static function insert(Struct_Db_PivotAuth_AuthSso $auth_mail):void {
		...
		$insert = [
			...
			"sso_auth_token" => $auth_mail->sso_auth_token,
	...
	public static function set(string $auth_map, array $set):int {
		...
		foreach ($set as $field => $_) {
			...
			if (!property_exists(Struct_Db_PivotAuth_AuthSso::class, $field)) {
	 ...
	 */
	public static function getOne(string $auth_map):Struct_Db_PivotAuth_AuthSso {
		...
		return Struct_Db_PivotAuth_AuthSso::rowToStruct($row);
	}
