...
/**
 * класс-интерфейс для таблицы pivot_rating_{10m}.message_answer_time_raw_list_{1}
 */
class Gateway_Db_PivotRating_MessageAnswerTimeRawList extends Gateway_Db_PivotRating_Main {

	protected const _TABLE_KEY = "message_answer_time_raw_list";
	 ...
	 * @return bool
	 */
	public static function isExistByUserIdAndAnswerAtAndConversationKey(int $user_id, int $answer_at, string $conversation_key):bool {

		try {
			self::getByUserIdAndAnswerAtAndConversationKey($user_id, $answer_at, $conversation_key);
	...
	/**
	 * Получаем одну запись по user_id, answer_at и conversation_key
	 *
	 * @param int    $user_id
	 * @param int    $answer_at
	 * @param string $conversation_key
	 *
	 * @return Struct_Db_PivotRating_MessageAnswerTimeRaw
	 * @throws \BaseFrame\Exception\Gateway\RowNotFoundException
	 */
	public static function getByUserIdAndAnswerAtAndConversationKey(int $user_id, int $answer_at, string $conversation_key):Struct_Db_PivotRating_MessageAnswerTimeRaw {

		$shard_key  = self::_getDbKey($user_id);
		$table_name = self::_getTableKey($user_id);

		// запрос проверен на EXPLAIN(INDEX=PRIMARY)
		$query = "SELECT * FROM `?p` WHERE `user_id` = ?i AND `answer_at` = ?i AND `conversation_key` = ?s LIMIT ?i";
		$row   = ShardingGateway::database($shard_key)->getOne($query, $table_name, $user_id, $answer_at, $conversation_key, 1);

		return self::_rowToStruct($row);
	}

	/**
	 * Получаем список записей по answer_at
	 *
	 * @param int $shard_id
	 ...
	 * @return array
	 */
	public static function getUserIdListBetweenAnswerAt(int $shard_id, int $date_start, int $date_end, int $limit, int $offset):array {

		$shard_key  = self::_getDbKey($shard_id);
		$table_name = self::_getTableKey($shard_id);

		// запрос проверен на EXPLAIN(INDEX=answer_at)
		$query = "SELECT DISTINCT(user_id) as `user_id` FROM `?p` FORCE INDEX (`answer_at`) WHERE `answer_at` BETWEEN ?i AND ?i LIMIT ?i OFFSET ?i";
	 ...
	 * @return array
	 */
	public static function makeInsertRow(int $user_id, int $answer_at, string $conversation_key, int $answer_time, int $space_id):array {
	 ...
	 */
	protected static function _rowToStruct(array $row):Struct_Db_PivotRating_MessageAnswerTimeRaw {

		if (!isset($row["user_id"])) {
			throw new \BaseFrame\Exception\Gateway\RowNotFoundException("not found row");
		}

		return Struct_Db_PivotRating_MessageAnswerTimeRaw::fromRow($row);
	}
}