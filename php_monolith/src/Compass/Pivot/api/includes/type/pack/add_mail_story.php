...
/**
 * класс для кодирования/декодирования сущности add_mail_story_map
 */
class Type_Pack_AddMailStory {

	// название упаковываемой сущности
	protected const _MAP_ENTITY_TYPE = "add_mail_story";
	...
	/**
	 * метод для запаковки и получения add_mail_story_map
	 *
	 * @throws \parseException
	 */
	public static function doPack(string $add_mail_story_id, string $shard_id, string $mail, int $created_at):string {
		...
		// формируем пакет
		$packet = [
			"add_mail_story_id" => $add_mail_story_id,
			"shard_id"          => $shard_id,
			"mail"              => $mail,
	 ...
	 */
	public static function getShardId(string $add_mail_story_map):string {

		$packet = self::_convertMapToPacket($add_mail_story_map);
		return $packet["shard_id"];
	}

	/**
	 * получить add_mail_story_id
	 *
	 * @throws \cs_UnpackHasFailed
	 */
	public static function getId(string $add_mail_story_map):string {

		$packet = self::_convertMapToPacket($add_mail_story_map);
		return $packet["add_mail_story_id"];
	}
	 ...
	 * @throws \cs_UnpackHasFailed
	 */
	public static function getMail(string $add_mail_story_map):string {
	...
	/**
	 * метод для расшифровки add_mail_story_key пользователя в add_mail_story_map
	 *
	 * @throws \cs_DecryptHasFailed
	 */
	public static function doDecrypt(string $add_mail_story_key):string {

		if (isset($GLOBALS["map_list"][$add_mail_story_key])) {
			return $GLOBALS["map_list"][$add_mail_story_key];
		}

		// расшифровываем
		$decrypt_result = openssl_decrypt($add_mail_story_key, ENCRYPT_CIPHER_METHOD, substr(ENCRYPT_KEY_PIVOT_SESSION, 0, openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD)), 0, substr(ENCRYPT_IV_PIVOT_SESSION, 0, openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD)));
		...
		// проверяем наличие ключа add_mail_story_map
		$decrypt_result = fromJson($decrypt_result);
		if (!isset($decrypt_result["add_mail_story_map"])) {
			throw new \cs_DecryptHasFailed();
		}

		// возвращаем auth_map
		$GLOBALS["map_list"][$add_mail_story_key] = $decrypt_result["add_mail_story_map"];
		return $decrypt_result["add_mail_story_map"];
	}

	/**
	 * метод для зашифровки в add_mail_story_map
	 *
	 */
	public static function doEncrypt(string $add_mail_story_map):string {

		if (isset($GLOBALS["key_list"][$add_mail_story_map])) {
			return $GLOBALS["key_list"][$add_mail_story_map];
		}

		// формируем массив для шифрования
		$arr = [
			"add_mail_story_map" => $add_mail_story_map,
		];

		// переводим сформированный add_mail_story_key в JSON
		$json = toJson($arr);

		// зашифровываем данные
		$add_mail_story_key = openssl_encrypt($json, ENCRYPT_CIPHER_METHOD, substr(ENCRYPT_KEY_PIVOT_SESSION, 0, openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD)), 0, substr(ENCRYPT_IV_PIVOT_SESSION, 0, openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD)));

		$GLOBALS["key_list"][$add_mail_story_map] = $add_mail_story_key;
		return $add_mail_story_key;
	}
	 ...
	 * @throws \cs_UnpackHasFailed
	 */
	protected static function _convertMapToPacket(string $add_mail_story_map):array {

		if (isset($GLOBALS["packet_list"][$add_mail_story_map])) {
			return $GLOBALS["packet_list"][$add_mail_story_map];
		}
		$packet  = self::_unpackAddMailStoryMap($add_mail_story_map);
		...
		$GLOBALS["packet_list"][$add_mail_story_map] = $output;
		return $output;
	}
	 ...
	 * @throws \cs_UnpackHasFailed
	 */
	protected static function _unpackAddMailStoryMap(string $add_mail_story_map):array {

		// получаем пакет из JSON
		$packet = fromJson($add_mail_story_map);
	...
	protected static function _getSignature(array $packet):string {
		...
		$sign     = hash_hmac("sha1", $string_for_sign, SALT_MAIL_ADDRESS);
