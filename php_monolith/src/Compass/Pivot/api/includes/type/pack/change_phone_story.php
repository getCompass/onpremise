...
/**
 * класс для кодирования/декодирования сущности change_phone_story_map
 */
class Type_Pack_ChangePhoneStory {

	// название упаковываемой сущности
	protected const _MAP_ENTITY_TYPE = "change_phone_story";
	...
	/**
	 * метод для запаковки и получения change_phone_story_map
	 *
	 * @throws \parseException
	 */
	public static function doPack(string $change_phone_story_id, string $shard_id, int $created_at):string {
		...
		// формируем пакет
		$packet = [
			"change_phone_story_id" => $change_phone_story_id,
			"shard_id"              => $shard_id,
	 ...
	 */
	public static function getShardId(string $change_phone_story_map):string {

		$packet = self::_convertMapToPacket($change_phone_story_map);
		return $packet["shard_id"];
	}

	/**
	 * получить change_phone_story_id
	 *
	 * @throws \cs_UnpackHasFailed
	 */
	public static function getId(string $change_phone_story_map):string {

		$packet = self::_convertMapToPacket($change_phone_story_map);
		return $packet["change_phone_story_id"];
	}
	...
	/**
	 * метод для расшифровки change_phone_story_key пользователя в change_phone_story_map
	 *
	 * @throws \cs_DecryptHasFailed
	 */
	public static function doDecrypt(string $change_phone_story_key):string {

		if (isset($GLOBALS["map_list"][$change_phone_story_key])) {
			return $GLOBALS["map_list"][$change_phone_story_key];
		}

		// расшифровываем
		$decrypt_result = openssl_decrypt($change_phone_story_key, ENCRYPT_CIPHER_METHOD, substr(ENCRYPT_KEY_PIVOT_SESSION, 0, openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD)), 0, substr(ENCRYPT_IV_PIVOT_SESSION, 0, openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD)));
		...
		// проверяем наличие ключа change_phone_story_map
		$decrypt_result = fromJson($decrypt_result);
		if (!isset($decrypt_result["change_phone_story_map"])) {
			throw new \cs_DecryptHasFailed();
		}

		// возвращаем auth_map
		$GLOBALS["map_list"][$change_phone_story_key] = $decrypt_result["change_phone_story_map"];
		return $decrypt_result["change_phone_story_map"];
	}

	/**
	 * метод для зашифровки в change_phone_story_map
	 *
	 */
	public static function doEncrypt(string $change_phone_story_map):string {

		if (isset($GLOBALS["key_list"][$change_phone_story_map])) {
			return $GLOBALS["key_list"][$change_phone_story_map];
		}
		...
		// переводим сформированный change_phone_story_key в JSON
		$json = toJson($arr);

		// зашифровываем данные
		$change_phone_story_key = openssl_encrypt($json, ENCRYPT_CIPHER_METHOD, substr(ENCRYPT_KEY_PIVOT_SESSION, 0, openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD)), 0, substr(ENCRYPT_IV_PIVOT_SESSION, 0, openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD)));

		$GLOBALS["key_list"][$change_phone_story_map] = $change_phone_story_key;
		return $change_phone_story_key;
	}
	 ...
	 * @throws \cs_UnpackHasFailed
	 */
	protected static function _convertMapToPacket(string $change_phone_story_map):array {

		if (isset($GLOBALS["packet_list"][$change_phone_story_map])) {
			return $GLOBALS["packet_list"][$change_phone_story_map];
		}
		$packet  = self::_unpackChangePhoneStoryMap($change_phone_story_map);
		...
		$GLOBALS["packet_list"][$change_phone_story_map] = $output;
		return $output;
	}

	/**
	 * распаковываем change_phone_story_map
	 *
	 * @throws \cs_UnpackHasFailed
	 */
	protected static function _unpackChangePhoneStoryMap(string $change_phone_story_map):array {

		// получаем пакет из JSON
		$packet = fromJson($change_phone_story_map);
