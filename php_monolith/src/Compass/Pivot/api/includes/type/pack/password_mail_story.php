 ...
 */
class Type_Pack_PasswordMailStory {

	// название упаковываемой сущности
	protected const _MAP_ENTITY_TYPE = "password_mail_story";
	...
	/**
	 * метод для запаковки и получения password_mail_story_map
	 *
	 * @throws \parseException
	 */
	public static function doPack(string $password_mail_story_id, string $shard_id, int $type, int $created_at):string {
	 ...
	 */
	public static function getShardId(string $password_mail_story_map):string {

		$packet = self::_convertMapToPacket($password_mail_story_map);
	 ...
	 * @throws \cs_UnpackHasFailed
	 */
	public static function getId(string $password_mail_story_map):string {

		$packet = self::_convertMapToPacket($password_mail_story_map);
		return $packet["password_mail_story_id"];
	}
	 ...
	 */
	public static function doDecrypt(string $password_mail_story_key):string {

		if (isset($GLOBALS["map_list"][$password_mail_story_key])) {
			return $GLOBALS["map_list"][$password_mail_story_key];
		}

		// расшифровываем
		$decrypt_result = openssl_decrypt($password_mail_story_key, ENCRYPT_CIPHER_METHOD, substr(ENCRYPT_KEY_PIVOT_SESSION, 0, openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD)), 0, substr(ENCRYPT_IV_PIVOT_SESSION, 0, openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD)));
		...
		if (!isset($decrypt_result["password_mail_story_map"])) {
			throw new \cs_DecryptHasFailed();
		}

		// возвращаем auth_map
		$GLOBALS["map_list"][$password_mail_story_key] = $decrypt_result["password_mail_story_map"];
		return $decrypt_result["password_mail_story_map"];
	}
	 ...
	 *
	 */
	public static function doEncrypt(string $password_mail_story_map):string {

		if (isset($GLOBALS["key_list"][$password_mail_story_map])) {
			return $GLOBALS["key_list"][$password_mail_story_map];
		}

		// формируем массив для шифрования
		$arr = [
			"password_mail_story_map" => $password_mail_story_map,
		];
		...
		$password_mail_story_key = openssl_encrypt($json, ENCRYPT_CIPHER_METHOD, substr(ENCRYPT_KEY_PIVOT_SESSION, 0, openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD)), 0, substr(ENCRYPT_IV_PIVOT_SESSION, 0, openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD)));

		$GLOBALS["key_list"][$password_mail_story_map] = $password_mail_story_key;
		return $password_mail_story_key;
	}
	 ...
	 * @throws \cs_UnpackHasFailed
	 */
	protected static function _convertMapToPacket(string $password_mail_story_map):array {

		if (isset($GLOBALS["packet_list"][$password_mail_story_map])) {
			return $GLOBALS["packet_list"][$password_mail_story_map];
		}
		$packet  = self::_unpackPasswordMailStoryMap($password_mail_story_map);
		...
		$GLOBALS["packet_list"][$password_mail_story_map] = $output;
		return $output;
	}
	 ...
	 * @throws \cs_UnpackHasFailed
	 */
	protected static function _unpackPasswordMailStoryMap(string $password_mail_story_map):array {

		// получаем пакет из JSON
		$packet = fromJson($password_mail_story_map);
