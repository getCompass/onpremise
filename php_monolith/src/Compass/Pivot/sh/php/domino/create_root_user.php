...
class Domino_CreateRootUser {
	...
	public function run(string $full_name, string $phone_number):void {
		...
		if ($root_user_id !== -1) {
			...
			console(yellowText("Root пользователь уже существует, id {$root_user_id}"));
			$authentication_token = Domain_Solution_Action_GenerateAuthenticationToken::exec($root_user_id);
			console(greenText("Код аутентификации:"));
			console($authentication_token);
			exit(0);
		}
		...
		try {
			...
			$authentication_token = Domain_Solution_Action_GenerateAuthenticationToken::exec($user_id);
			...
			console(greenText("Код аутентификации:"));
			console($authentication_token);
			console(yellowText("ВАЖНО: Код аутентификации может быть использован только после создания команды для общения. Создайте команду, чтобы авторизоваться в приложении и начать работу."));
			exit(0);
		}
