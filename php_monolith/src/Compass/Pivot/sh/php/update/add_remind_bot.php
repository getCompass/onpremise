...
/**
 * Скрипт для добавления бота Напоминания во все компании
 */
class Add_Remind_Bot {
	...
	protected function _updateCompany(Struct_Db_PivotCompany_Company $company):void {
		...
		if ($this->_is_dry_run === false) {
			...
			Gateway_Socket_Company::addSystemBot($bot_info, $company->company_id, $company->domino_id, Domain_Company_Entity_Company::getPrivateKey($company->extra));
		}
	}
...
(new Add_Remind_Bot())->run(Add_Remind_Bot::getCompanyListFromCli(), isDryRun());
