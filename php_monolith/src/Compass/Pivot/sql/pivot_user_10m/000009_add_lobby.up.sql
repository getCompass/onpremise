...
ALTER TABLE `user_company_list_1` DROP COLUMN `status_alias`;
ALTER TABLE `user_company_list_1` DROP COLUMN `can_login_alias`;
ALTER TABLE `user_company_list_1` ADD COLUMN `entry_id` INT(11) NOT NULL DEFAULT '0' COMMENT 'идентификатор с которым пользователь попал в компанию' AFTER `order`;
ALTER TABLE `user_company_list_1` ADD COLUMN `extra` JSON NOT NULL DEFAULT (JSON_ARRAY()) COMMENT 'extra пользователя' AFTER `updated_at`;
...
ALTER TABLE `user_company_list_1` DROP COLUMN `status`;
DROP INDEX `company_id_status` ON `user_company_list_1`;
CREATE INDEX `user_id_and_order` ON `user_company_list_1` (`user_id`, `order` DESC);