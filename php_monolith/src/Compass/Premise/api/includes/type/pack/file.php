...
class Type_Pack_File {
	...
	protected const _PACKET_SCHEMA = [
		...
		// 3 версия появилась из-за перехода cloud в domino
		3 => [
			"server_type" => "a",
			"shard_id"    => "b",
			"table_id"    => "c",
	...
	public static function doDecrypt(string $file_key, string|false $encrypt_iv = false, string|false $encrypt_key = false):string {
		...
		$encrypt_iv  = $encrypt_iv ?: ENCRYPT_IV_DEFAULT;
		...
		// проверяем принадлежность файла к pivot
		if (self::getServerType($decrypt_result["file_map"]) !== self::_PREFIX_KEY_PIVOT) {
			throw new \cs_DecryptHasFailed();
		}
