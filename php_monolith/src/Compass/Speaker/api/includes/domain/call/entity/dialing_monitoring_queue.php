...
/**
 * Класс для работы с очередями мониторинга dialing-соединения
 */
class Domain_Call_Entity_DialingMonitoringQueue {

	public const MONITORING_FINISH_REASON = CALL_FINISH_REASON_IGNORED;
	...
	public static function getList(int $limit, int $offset):array {
		...
		return Gateway_Db_CompanyCall_CallMonitoringDialing::getAll($limit, $offset);
	}
	...
	public static function deleteForUsers(array $user_id_list, string $call_map):void {
		...
		Gateway_Db_CompanyCall_CallMonitoringDialing::deleteForUsers($user_id_list, $call_map);
	}
	...
	public static function getUsersWhoNeedFinishCall(array $user_id_list, array $call_map_list):array {
		...
		foreach ($meta_list as $meta_row) {
			...
			foreach ($user_id_list as $user_id) {
				...
				// проверяем, может быть звонок уже завершен или пользователь успел взять трубку
