 ...
 */
class Type_Pack_Preview {

	// название упаковываемой сущности
	protected const _MAP_ENTITY_TYPE = "preview";
	...
	public static function doEncrypt(string $preview_map):string {
		...
		if (isset($GLOBALS["key_list"][$preview_map])) {
			return $GLOBALS["key_list"][$preview_map];
		}
		...
		$preview_key = openssl_encrypt($json, ENCRYPT_CIPHER_METHOD, ENCRYPT_KEY_DEFAULT, 0, $iv);

		$GLOBALS["key_list"][$preview_map] = $preview_key;
		return $GLOBALS["key_list"][$preview_map];
	}
	...
	public static function doDecrypt(string $preview_key):string {
		...
		if (isset($GLOBALS["map_list"][$preview_key])) {
			return $GLOBALS["map_list"][$preview_key];
		}

		// расшифровываем
		$iv_length      = openssl_cipher_iv_length(ENCRYPT_CIPHER_METHOD);
		$iv             = substr(ENCRYPT_IV_DEFAULT, 0, $iv_length);
		$decrypt_result = openssl_decrypt($preview_key, ENCRYPT_CIPHER_METHOD, ENCRYPT_KEY_DEFAULT, 0, $iv);
		...
		$GLOBALS["map_list"][$preview_key] = $decrypt_result["preview_map"];
	...
	protected static function _convertMapToPacket(string $preview_map):array {
		...
		if (isset($GLOBALS["packet_list"][$preview_map])) {
			return $GLOBALS["packet_list"][$preview_map];
		}
		...
		$GLOBALS["packet_list"][$preview_map] = $output;
		return $output;
	}
	...
	protected static function _getSignature(array $packet):string {
		...
		$sign     = hash_hmac("sha1", $string_for_sign, SALT_PACK_PREVIEW[$version]);
