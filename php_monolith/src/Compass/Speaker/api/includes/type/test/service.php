...
class Type_Test_Service {
	...
	public const PATH = PUBLIC_ENTRYPOINT_GO_TEST . "/";
	...
	// получаем статус ws коннекта
	public static function getOffer(string $connection_uuid):array {

		$method = "calls.getOffer";
		$data   = [
	...
	public static function createPublisher(int $user_id, string $connection_uuid):array {
		...
		return [
			"offer" => [
				"sdp"  => $response["response"]["offer"]["sdp"],
				"type" => $response["response"]["offer"]["type"],
			],
		];
	}
	...
	public static function createSubscriber(int $user_id, string $connection_uuid, array $offer):array {
		...
		return [
			"answer" => [
				"sdp"  => $response["response"]["answer"]["sdp"],
				"type" => $response["response"]["answer"]["type"],
			],
		];
	}
