...
/**
 * Action для создания треда для заявки найма
 */
class Domain_Thread_Action_AddToHiringRequest {

	/**
	 * добавляем тред к заявке найма
	 * @long
	 */
	public static function do(int $user_id, int $hiring_request_id, bool $is_need_thread_attach = false):array {

		// проверяем, может тред у заявки уже имеется
		$response = Gateway_Socket_Company::getMetaForCreateThread($user_id, $hiring_request_id, "hiring_request");
		if ($response["is_exist"] == 1) {
			return Type_Thread_Meta::getOne($response["thread_map"]);
		}
		$hiring_request_created_at = $response["created_at"];      // время создания заявки
		$creator_user_id           = $response["creator_user_id"]; // создатель заявки

		// для заявки создателя компании нельзя прицепить тред
		if (isset($response["extra"]["extra"]["is_company_creator"]) && $response["extra"]["extra"]["is_company_creator"]) {
			throw new cs_HiringRequestIsNotAllowedForAddThread("thread is not allowed for compnay creator hiring request");
		}

		// получаем данные чата найма/увольнения
		$conversation_data = Gateway_Socket_Conversation::getDataForCreateThreadOnHireRequest();

		// собираем создателя заявки и треда (это могут быть разные люди)
		$user_id_list = array_unique([$user_id, $creator_user_id]);
		...
		$thread_meta_row = self::_createThread(
			(string) $hiring_request_id,
			$conversation_data["conversation_map"],
			$users,
			$user_id,
			$user_id,
			$hiring_request_created_at,
			false
		);

		// сокет-запрос для закрепления thread_map за заявкой
		if ($is_need_thread_attach) {
			Gateway_Socket_Company::setThreadMapForHireRequest($hiring_request_id, "hiring_request", $thread_meta_row["thread_map"]);
		}

		// отправляем события о создании треда к заявке найма
		self::_sendEventOnThreadAdd($thread_meta_row, $hiring_request_id, $thread_meta_row["users"]);
	...
	/**
	 * создаем тред для заявки найма/увольнения
	 */
	protected static function _createThread(string $hiring_request_id, string $conversation_map,
		...
		$parent_rel = Type_Thread_ParentRel::create(
			PARENT_ENTITY_TYPE_HIRING_REQUEST,
			$creator_user_id,
			$hiring_request_id,
			$request_created_at,
			[]
	...
	/**
	 * отправляем ws-события при создании треда у заявки найма
	 */
	protected static function _sendEventOnThreadAdd(array $thread_meta_row, int $hiring_request_id, array $users):void {

		// формируем список пользователей для отправки в go_sender
		$talking_user_list = Type_Thread_Meta_Users::getTalkingUserList($users);

		$routine_key = sha1((string) $hiring_request_id);
		...
		// отправляем ивент о прикреплении треда к заявке найма
		Gateway_Bus_Sender::hiringRequestThreadAttached($talking_user_list, $hiring_request_id, $thread_meta_row, $thread_meta_row["thread_map"], $routine_key);
