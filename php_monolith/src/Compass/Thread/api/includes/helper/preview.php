...
class Helper_Preview {
	...
	protected static function _createSimplePreview(string $prepared_url, string $domain):array {
		...
		$final_url              = Type_Preview_Utils::encodeUrl($final_url);
	...
	// получаем информацию по url preview
	protected static function _makePreviewData(\Curl $curl, string $html, string $domain, int $user_id, string $prepared_url):array {
		...
		if (in_array($mime_type, self::_IMAGE_MIME_TYPE_LIST)) {
			...
			if (IS_IMAGE_URL_PREVIEW_ENABLED === false) {
				throw new cs_UrlNotAllowToParse($prepared_url);
			}
			return self::_makeImageDataPreview($domain, $user_id, $prepared_url, $is_need_favicon);
		}
	...
	public static function makeData(string $user_id, string $url, string $short_url, string $html):array {
		...
		if (str_contains($url, Type_Preview_Parser_Compass::DOMAIN)
			...
			|| preg_match(sprintf("/%s(\/|$)/iu", PIVOT_DOMAIN), $url)) {
	...
	public static function attachLinkList(int    $user_id, string $message_map, string $parent_conversation_map, array $users, array $link_list,
		...
		self::_sendWS($users, $message_map, $link_list, $threads_updated_version, $preview_map, $preview_type, $preview_image);
	}
	...
	protected static function _getDomain(string $url):string {
		...
		if ($domain === false || is_null($domain)) {
			...
			throw new Domain_Thread_Exception_Preview_IncorrectUrl("Failed to get domain from url: {$url}");
		}
