...
// класс для парсинга ссылок на instagram.com
class Type_Preview_Parser_Instagram extends Type_Preview_Parser_Helper implements Type_Preview_Parser_Interface {

	public const DOMAIN = "instagram.com";

	protected const _TYPE_PHOTO = "instapp:photo";
	protected const _SITE_NAME  = "Instagram";
	...
	public static function isRelevantUrl(string $url):bool {
		...
		return inHtml($url, Type_Preview_Parser_Instagram::DOMAIN) || ServerProvider::isTest() && inHtml($url, mb_strtolower(self::_SITE_NAME));
	}
	...
	protected static function _isContent(string $type):bool {
		...
		return $type == self::_CONTENT_TYPE_WEBSITE || $type == self::_CONTENT_TYPE_VIDEO || $type == self::_TYPE_PHOTO;
	}
