...
class Type_Preview_Worker {
	 ...
	 * @long
	 */
	public function doWork(string $message_map, int $user_id, array $url_list, string $lang, array $user_list, bool $need_full_preview, string $parent_conversation_map):void {

		$link_list         = [];
		$prepared_url_list = [];

		if ($url_list === [] || $message_map === "") {
			return;
		}
	...
	protected function _createPreview(string $original_url, string $prepared_url, int $user_id, string $lang, bool $need_full_preview = false):array {
		...
		} catch (Domain_Thread_Exception_Preview_IncorrectUrl) {
			return [null, [], []];
		}
	}
