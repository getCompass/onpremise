...
class Type_Task_Attribute_Executor {
	...
	public function __construct(string $task_type, string $struct_data_class) {
		...
		if (!class_exists($struct_data_class)) {
			throw new ParseFatalException("passed non-existing class");
		}

		if (!is_subclass_of($struct_data_class, Struct_Default::class)) {
			throw new ParseFatalException("passed class is not struct");
		}
