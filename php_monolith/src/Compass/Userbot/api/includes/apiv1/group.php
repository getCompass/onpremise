 ...
 * Class ApiV1_Group
 */
class Apiv1_Group extends Apiv1_Default {
	...
	public function send():array {
		...
		try {
			$request_id = Domain_Group_Scenario_Api_V1::send($this->token, $group_id, $type, $text, $file_id);
	...
	public function getList():array {
		...
		try {
			...
			$request_id = Domain_Group_Scenario_Api_V1::getList($this->token, $count, $offset);
		} catch (\cs_Userbot_RequestIncorrect $e) {
			throw new CaseException(CASE_EXCEPTION_CODE_1000, "invalid parameters passed: " . $e->getMessage());
		} catch (\cs_Userbot_NotFound) {
			throw new CaseException(CASE_EXCEPTION_CODE_2, "userbot is not found");
