...
class Apiv1_Handler extends Api implements \RouteHandler {
	...
	public function handle(string $api_method, array $post_data, int $user_id = 0):array {
		...
		try {
			$output = self::_doStart($api_method, $post_data);
	 ...
	 */
	protected static function _doStart(string $api_method, array $post_data):array {

		// проверяем дефолт поля в запросе
		self::_checkDefaultFields($post_data);

		// получаем payload
		$payload = self::_getPayload($post_data);

		// проверяем валидность подписи
		self::_throwIfPassedInvalidSignature($post_data["signature"]);

		// находим бота, от которого делается запрос
		try {
			$userbot = Userbot::init($post_data["token"]);
		...
		self::_throwIfSignatureIsNotAgreed($post_data["signature"], $payload, $userbot->token, $userbot->secret_key);

		// делаем запрос и получаем ответ
		try {
			$response = self::_getResponse($api_method, $payload, $userbot);
	...
	/**
	 * Получаем payload из json в запросе
	 *
	 ...
	 * @throws CaseException
	 */
	protected static function _getPayload(array $post_data):array {

		// проверяем, что в payload передана именно строка
		if (!is_string($post_data["payload"])) {
			throw new CaseException(CASE_EXCEPTION_CODE_1, "wrong payload format");
		}

		// если первый символ строки не { — это не json
		if (mb_substr($post_data["payload"], 0, 1) != "{") {
			throw new CaseException(CASE_EXCEPTION_CODE_1, "wrong payload format");
		}

		// получаем payload из строки 1 раз. Нечего парсить json несколько раз за время выполнения запроса - это дорогая операция
		return fromJson($post_data["payload"]);
	}
	 ...
	 * @throws CaseException
	 */
	protected static function _checkDefaultFields(array $post_data):void {

		// если отсутствуют обязательные поля
		if (!isset($post_data["token"], $post_data["signature"], $post_data["payload"])) {
			throw new CaseException(CASE_EXCEPTION_CODE_1, "missing required fields");
		}

		// если поля, которые не должны быть пустыми, пусты
		if (isEmptyString($post_data["token"]) || isEmptyString($post_data["signature"])) {
			throw new CaseException(CASE_EXCEPTION_CODE_1, "required fields are empty");
		}
	}
	 ...
	 * @throws \blockException
	 */
	protected static function _throwIfSignatureIsNotAgreed(string $signature, array $payload, string $token, string $secret_key):void {

		// формируем и сверяем signature
		$formed_signature = Type_Userbot_Main::getApiV1Signature($payload, $token, $secret_key);
