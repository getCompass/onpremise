...
abstract class Domain_User_Scenario_Api {
	...
	public static function send(string $token, int $user_id, string $type, string|false $text, string|false $file_id):string {
		...
		try {
			...
			$message_key = Gateway_Socket_Conversation::sendMessageToUser($userbot->userbot_user_id, $user_id, $text, $file_id, $userbot->company_url);
	...
	public static function getList(string $token, int $count, int $offset):string {
		...
		try {
			...
			$user_list = Gateway_Socket_Company::getUserList($userbot->userbot_id, $count, $offset, $userbot->company_url);
