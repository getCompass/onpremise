...
class Domain_Userbot_Action_SendWebhookAnswer {
	...
	protected static function _messageSend(Struct_Userbot_Info $userbot, string $group_id, int $user_id, array $answer_post):void {
		...
		if (mb_strlen($group_id) > 0) {
			Gateway_Socket_Conversation::sendMessageToGroup($userbot->userbot_user_id, $group_id, $text, $file_id, $userbot->company_url);
		} else {
			Gateway_Socket_Conversation::sendMessageToUser($userbot->userbot_user_id, $user_id, $text, $file_id, $userbot->company_url);
		}
	}
	...
	protected static function _threadSend(Struct_Userbot_Info $userbot, string $message_id, array $answer_post):void {
		...
		Gateway_Socket_Thread::sendMessageToThread($userbot->userbot_user_id, $message_id, $text, $file_id, $userbot->company_url);
	}
	...
	protected static function _messageAddReaction(Struct_Userbot_Info $userbot, string $message_id, array $answer_post):void {
		...
		Domain_Message_Action_AddReaction::do($userbot->userbot_user_id, $message_id, $reaction, $userbot->company_url);
	}
}