...
class Type_Antispam_Ip {
	...
	protected const _DB_KEY    = "userbot_service";
	protected const _TABLE_KEY = "antispam_ip";

	const USERBOT_REQUEST = [
		"key"    => "USERBOT_REQUEST",
		"limit"  => 50,
		"expire" => HOUR1,
	];
	...
	public static function checkAndIncrementBlock(array $block_key):void {
		...
		if ($row["count"] >= $block_key["limit"]) {
			throw new \BaseFrame\Exception\Request\CaseException(CASE_EXCEPTION_CODE_5, "error limit reached while sending request - please try again later");
		}

		// обновляем запись
		self::_set($row["ip_address"], $row["key"], $row["count"] + 1, $row["expire"]);
	}
	...
	public static function setByKey(string $key, string $ip_address, int $count, int $expire):void {
		...
		self::_set($ip_address, $key, $count, $expire);
	}
	 ...
	 *
	 */
	protected static function _set(string $ip_address, string $key, int $count, int $expire):void {
