...
/**
 * класс для форматирования пост параметров
 */
class Type_Formatter {
	...
	public static function post(array $post_data, string $type, string $key, $default = null) {
		...
		if (!isset($post_data[$key])) {
			...
			throw new CaseException(CASE_EXCEPTION_CODE_8, "There is no required parameter ({$key}) in request, or developer did not passed default value for it.");
		}
		...
		if (!is_array($value)) {
			...
			$type = gettype($value);
			throw new CaseException(CASE_EXCEPTION_CODE_8, "passed unknown param type ({$type})");
		}
	...
	protected static function _formatScalar(string $type, mixed $value):mixed {
		...
		return match ($type) {
			...
			default => throw new CaseException(CASE_EXCEPTION_CODE_8, "Developer passed not existed type ({$type}) of expecting parameter! Use types " . implode(", ", self::_ALLOWED_SCALAR_TYPES)),
		};
	}
	...
	protected static function _getInt(string $value):int {
		...
		throw new CaseException(CASE_EXCEPTION_CODE_8, "Expecting value ({$value}) to be int, but got string");
	}
	...
	protected static function _getFloat(string $value):float {
		...
		throw new CaseException(CASE_EXCEPTION_CODE_8, "Expecting value ({$value}) to be float, but got string");
	}
	...
	protected static function _getHash(string $value):string {
		...
		if (strlen($value) != 32 && strlen($value) != 40) {
			...
			$length = strlen($value);
			throw new CaseException(CASE_EXCEPTION_CODE_8, "Expecting hash ({$value}), but its length ({$length}) not equal 32 nor 40 characters!");
		}
	...
	protected static function _getUuid(string $value):string {
		...
		if (!isUuidValid($string_value)) {
			throw new CaseException(CASE_EXCEPTION_CODE_8, "not a valid uuid");
		}
	...
	protected static function _formatArray(string $type, array $value):array {
		...
		return match ($type) {
			...
			default => throw new CaseException(CASE_EXCEPTION_CODE_8, "Developer passed not existed type ({$type}) of expecting parameter! Use types " . implode(", ", self::_ALLOWED_ARRAY_TYPES)),
		};
	}
	...
	protected static function _getArrayOfInt(array $value):array {
		...
		foreach ($value as $k => $v) {
			...
			if (!is_int($k) && !is_scalar($v)) {
				throw new CaseException(CASE_EXCEPTION_CODE_8, "not a valid array int");
			}
	...
	protected static function _resolveType(mixed $value):string {
		...
		return match ($type) {
			...
			default => throw new CaseException(CASE_EXCEPTION_CODE_8, "Developer passed not existed type ({$type}) of expecting parameter!"),
		};
	}
}