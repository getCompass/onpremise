...
class Type_System_Monitoring {
	...
	// ключ для datastore
	#[Pure] protected static function _getKey(string $file_path):string {
	...
	protected static function _parseLogs(string $path, int $cursor):void {
		...
		foreach (self::_yieldOneLine($path, $cursor) as $line) {
			...
			// если количество логов превысило лимит, то отправляем в чат с ошибками
	...
	protected static function _send(array $logs):void {
		...
		if (!defined("NOTICE_CHANNEL_EXCEPTION") || isEmptyString(NOTICE_CHANNEL_KEY)) {
			return;
		}
